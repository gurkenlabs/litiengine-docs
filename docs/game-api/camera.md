...
| :--- | :--- | :--- |
| `setFocus(Point2D point)` | `void` | Centers camera viewport directly on the given map coordinates (e.g. `entity.getCenter()`). |
| `setFocus(double x, double y)` | `void` | Centers camera viewport directly on the specified world coordinates. |
...
| `shake(double intensity, int delay, int duration)` | `void` | Triggers a screen-shake trauma effect with specified delay and tick duration. |
...
public class PlayerCameraFollower implements IUpdateable {
  ...
  public PlayerCameraFollower(ICombatEntity player) {
    this.player = player;
  }
